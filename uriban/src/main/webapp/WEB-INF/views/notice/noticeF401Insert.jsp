				...
				<div class="card">
					<div class="card-header d-flex justify-content-between">
						<div class="header-title">
							<h4 class="card-title">가정통신문 -> 공지 게시판 등록</h4>
							<a href="../notice/templateList?memberId=${p.memberId }" class="ri-edit-box-line m-0"> 양식 가져오기</a>
						</div>
					</div>
					<div class="card-body">
						<p>학생분들과 학부모분들께 전달하고 싶은 가정통신문을 게시판에 작성하는 페이지 입니다<br>
							(공지 등록 시 클래스 전원에게 자동으로 회람 톡이 전송됩니다.)</p>
						<form class="was-validated" name="insertForm" action="../notice/noticeF401Insert?${_csrf.parameterName}=${_csrf.token}" accept-charset="UTF-8" method="post" enctype="multipart/form-data">
							<div class="form-group">
								<select class="custom-select" required name="noticeType">
									<option value="F401" selected="selected">가정통신문</option>
								</select>
								<div class="invalid-feedback">가정통신문 등록에선 가정통신문만 등록이 가능합니다.</div>
							</div>
							...
							<div class="mb-3">
								...
								<div class="invalid-feedback">우리반공지에 전달할 가정통신문 제목을 작성해 주세요.</div>
							</div>
							...
							<div class="mb-3">
								...
								<div class="invalid-feedback">우리반공지에 전달할 가정통시문 게시판 내용을 작성해 주세요!</div>
							</div>
							...
							<div class="custom-control custom-checkbox mb-3">
								...
								<label class="custom-control-label" for="customControlValidation1">
									지금 작성된 공지사항을 개인 템플릿에 저장하려해요!</label>
								<br>
								<small>지금 작성되어 있는 공지 유형과 제목, 내용을 
							...
							<button type="submit" id="addBtn" class="btn btn-primary"> 가정통신문 게시판 등록 </button>
				    ...
				    <div class="alert text-white bg-info layer layer2" id="tem" role="alert">
						...
						<script>
							...
							$('[name="insertForm"]').on('submit', function(e){
								...
								$('.done').text('우리반 공지에 가정통신문이 등록 되었습니다.');
								$('#done').show();
								setTimeout(function(){
									$('.layer1').hide();
								}, 10000)
